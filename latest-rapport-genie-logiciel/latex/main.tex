...
\newtcolorbox{codebox}{
  colback=CodeBg, colframe=HEEClight, boxrule=0pt, leftrule=2pt, arc=0pt,
  left=8pt, right=8pt, top=4pt, bottom=4pt, breakable
}

% ===== LISTINGS =====
\lstset{basicstyle=\ttfamily\small, backgroundcolor=\color{CodeBg}, frame=none, breaklines=true, breakatwhitespace=true, showstringspaces=false, tabsize=2, xleftmargin=1em, xrightmargin=1em}
...
Les perspectives d'évolution de SaaS Directory sont nombreuses. L'intégration complète de Stripe pour le paiement (le schéma contient déjà les champs \texttt{stripeCustomerId} et \texttt{stripeSubscriptionId}), l'ajout d'un système de recherche avancée, et le déploiement sur une infrastructure cloud avec Docker et Kubernetes constituent les axes de développement futurs.
\newpage
